...
function getArtistByIndex(ArrayT3, numT3) {
  /*Your Code Here*/
  return `the artist at index ${numT3} is ${ArrayT3[numT3].name}`;
}
...
function get20s(arrayT4) {
  /*Your Code Here*/
  let newArrayT4 = [];
  for(let i = 0; i < arrayT4.length; i++){
      let newYearSplit = arrayT4[i].years;
      newYearSplit.split(' ', 3);
      if(newYearSplit.startsWith("19")){
        newArrayT4.push(arrayT4[i].name);
      }
  }
  console.log(newArrayT4);
  return newArrayT4;
...
function removeArtist(arrayT5, numT5,) {
  /*Your Code Here*/
  arrayT5.splice(numT5,1);
  return arrayT5.length;
}
...
function addArtist(ArrayT6) {
  /*Your Code Here*/
  ArrayT6.push({ id: 20,name: "Bradley Michaels", years: 1992 - 2021, genre: "Web Design", nationality: "American",bio: "Avid lover of the Mets and Islanders. When not catching sporting events at stadiums Im at a concert!",
  })
  return ArrayT6;
}
...
function lotsOfArt(arrayT7) {
  /*Your Code Here*/
  let art100 = [];
  for(let i= 0; i< arrayT7.length; i++){
    if(arrayT7[i].paintings > 100){
      art100.push(arrayT7[i].name);
    }
  }
  return art100;
}
